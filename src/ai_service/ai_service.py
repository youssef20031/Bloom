...
def initialize_ai_components():
    """
    Loads the PRE-BUILT vector store from disk (created with local embeddings)
    and initializes the RAG chain for answering questions.
    """
    global vector_store, rag_chain, llm
    print("--- Initializing AI Components from Persistent Storage ---")

    # We still need the Google API key for the CHAT model
    if 'GOOGLE_API_KEY' not in os.environ:
        print("ERROR: GOOGLE_API_KEY environment variable not set for the chat model.")
        return

    # --- CHANGE: Use the same free, local embedding model to load the database ---
    print("Initializing local embedding model to load the database...")
    model_name = "sentence-transformers/all-MiniLM-L6-v2"
    embeddings = HuggingFaceEmbeddings(model_name=model_name)
    print("Local embedding model loaded.")

    # Define the path to the database you created with the build script
    persist_directory = "src/ai_service/chroma_db_persistent"

    if not os.path.exists(persist_directory):
        print(f"ERROR: Persistent vector store not found at '{persist_directory}'.")
        print("Please run the '1-build_database.py' script first to create it.")
        return
    ...
    # 3. Build the RAG chain (this part remains the same, using Google for the chat LLM)
    print("Building the conversational RAG chain with Gemini...")
...
def build_pdf_from_report(report_text: str) -> BytesIO:
    buffer = BytesIO()
    doc = SimpleDocTemplate(buffer, pagesize=A4, rightMargin=inch * 0.75, leftMargin=inch * 0.75, topMargin=inch * 0.75,
                            bottomMargin=inch * 0.75)

    styles = getSampleStyleSheet()
    styles.add(ParagraphStyle(name='SectionHeading', parent=styles['h2'], spaceBefore=12, spaceAfter=6,
                              textColor=colors.HexColor('#007DB8')))
